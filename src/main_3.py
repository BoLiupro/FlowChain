...
def compute_region_probability_from_samples(sampled_seq, polygon):
    """
    sampled_seq: numpy array, shape (N_samples, T, 2)
                 预测采样轨迹
    ...
    for t in range(T):
        pts_t = sampled_seq[:, t, :]   # (N, 2)
        count = sum(polygon.contains(Point(x, y)) for x, y in pts_t)
        region_prob_seq.append(count / N)

    return np.array(region_prob_seq)
...
def evaluate_model(cfg: CfgNode, model: torch.nn.Module, data_loader: torch.utils.data.DataLoader, visualize=False):
    ...
    if visualize:
        with torch.no_grad():
            ...
            for i, data_dict in enumerate(tqdm(data_loader_one_each, leave=False, total=10)):
                ...
                region_prob_seq = compute_region_probability_from_samples(
                    sampled_xy, CROSSING_POLY
                )   # (T, ), 0~1
    ...
    #         os.makedirs("plot/obs", exist_ok=True)
    #         os.makedirs("plot/gt", exist_ok=True)
    #         os.makedirs("plot/pred", exist_ok=True)
    #         # 清空之前的文件
    #         for f in os.listdir("plot/obs"):
    #             os.remove(os.path.join("plot/obs", f))
    #         for f in os.listdir("plot/gt"):
    #             os.remove(os.path.join("plot/gt", f))
    #         for f in os.listdir("plot/pred"):
    #             os.remove(os.path.join("plot/pred", f))

    #         for idx, sample in enumerate(dict_list):
    #             obs  = sample["obs"].cpu().numpy()
    #             gt   = sample["gt"].cpu().numpy()
    #             pred = sample[('pred', 0)].cpu().numpy()

    #             np.savetxt(f"plot/obs/obs_{idx}.txt",  obs.reshape(-1, obs.shape[-1]))
    #             np.savetxt(f"plot/gt/gt_{idx}.txt",   gt.reshape(-1, gt.shape[-1]))
    #             np.savetxt(f"plot/pred/pred_{idx}.txt", pred.reshape(-1, pred.shape[-1]))
